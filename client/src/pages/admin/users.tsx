...
export default function AdminUsers() {
  ...
  const handleToggleCodAccess = async (userId: number, currentCodEnabled: boolean) => {
    try {
      const token = localStorage.getItem('adminToken');
      if (!token) {
        throw new Error('No authentication token found');
                      ...
                      <TableRow key={user.id}>
                        ...
                        <TableCell>
                          <div className="flex items-center gap-2">
                            ...
                            >
                              {user.codEnabled !== false ? 'Enabled' : 'Disabled'}
                            </span>
                            <Button
                              variant="ghost"
                              size="sm"
                              onClick={() => handleToggleCodAccess(user.id, user.codEnabled !== false)}
