...
class SmsService {
   ...
   * Send OTP via SMS
   */
  async sendOTP(mobile: string, purpose: 'registration' | 'password_reset'): Promise<{ success: boolean; message: string }> {
    try {
   ...
   * Verify OTP
   */
  async verifyOTP(mobile: string, otp: string, purpose: 'registration' | 'password_reset'): Promise<{ success: boolean; message: string }> {
    try {
      const now = new Date();
